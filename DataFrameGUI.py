...
class DataFrameSortFilterProxyModel(QSortFilterProxyModel):
    ...
    def _source_model_changed(self):
        ...
        print(f"SOURCE MODEL CHANGED{len(self._accepted_rows)}")
...
class DataFrameWidget(QTableView):
    ...
    def make_cell_context_menu(self, menu, row_ix, col_ix):
        ...
        def _to_excel():
            ...
            try:
                self.df.to_excel(self.defaultExcelFile, sheet_name=self.defaultExcelSheet)
...
if __name__ == '__main__':
    ...
    _app = App(sys.argv)
    data = pandas.read_csv('d:\\Projects\\AutoForm\\docs\\01_PV_R8_Brace_Sill_OP10_FV1_D-20_31_Sheet_ElementData.csv')
    app = DataFrameApp(data)
    app.show()
    _app.exec_()
    ...
    # _app.loadStyleSheet(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'stylesheets', 'coffee'))
